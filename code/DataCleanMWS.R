#CREATE and CLEAN time series for raw spread rate, climate variable (tmean, tmmin, ppt, and snow_depth),
#and climate index data. These time series are the input for the wavelet analyses methods employed in this
#manuscript.

#Read in combined spread rate + associated climate variable data (tmean, tmin, ppt, and snow_depth) 
#for the Mixed Wood Shield (MWS) ecoregion
in.data.t <- read.csv(here("data", "mws_spread_climate.csv"), header = TRUE, sep = ",")
...
in.data.climate.indices <- read.csv(here("data", "clim.indices.mws.winter.csv"), header = TRUE, sep = ",")
...
# PDO time series for MWS
in.data.pdo <- in.data.t[,c("bearing2", "year_t", "pdo.winter.mean")]
...
# ENSO (MEI index) time series for MWS
in.data.enso <- in.data.t[,c("bearing2", "year_t", "enso.winter.mean")]
...
# NAO time series for MWS
in.data.nao <- in.data.t[,c("bearing2", "year_t", "nao.winter.mean")]
